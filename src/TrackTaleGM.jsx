...
import { LayoutDashboard, ListChecks, Building2, Calendar as CalIcon, Settings, Search, Bell, Plus, ChevronRight, ChevronDown, AlertCircle, Clock, CheckCircle2, ArrowLeft, MessageSquare, FileText, Upload, Check, User, GitBranch, Table2, ChevronLeft, LayoutGrid, Rows3, AlertTriangle, Trash2, X } from "lucide-react";
...
// Auto-cascade: real TNA offsets derived from mentor's Excel (cumulative days from fabric commit)
const cascadeDates = (fabricCommit, buyer, ih) => {
  if (!fabricCommit) return {};
  const xftyDays = BUYERS[buyer]?.xftyDays || 49;
  return {
    "fabric-commitment": fabricCommit,
    "fabric-ordered": addDays(fabricCommit, 1),
    "lab-dip": addDays(fabricCommit, 5),
    "fit-sample": addDays(fabricCommit, 12),
    "fob-submission": addDays(fabricCommit, 73),
    "fob-approval": addDays(fabricCommit, 80),
    "fpt": addDays(fabricCommit, 84),
    "gpt": addDays(fabricCommit, 89),
    "pp-sample": addDays(fabricCommit, 101),
    "pcd": addDays(fabricCommit, 108),
    "ex-factory": ih ? addDays(ih, -xftyDays) : addDays(fabricCommit, 115),
    "gac": ih ? addDays(ih, -4) : addDays(fabricCommit, 130),
    "in-house": ih || addDays(fabricCommit, 134),
  };
};
...
const INIT_ORDERS = [
  {
    ...
    milestones: [
      ...
      mkMs("PP sample", "2026-09-29", "Factory/Buyer", "upcoming", 1, [
        { label: "R1 Sub", date: "2026-09-29", status: "pending" }, { label: "R1 App", date: "2026-10-05", status: "pending" },
        { label: "R2 Sub", date: "2026-10-06", status: "pending" }, { label: "R2 App", date: "2026-10-12", status: "pending" },
      ]),
      mkMs("PCD", "2026-10-06", "Factory", "upcoming"),
      mkMs("Ex-factory", "2026-10-01", "Factory", "upcoming"),
      mkMs("GAC", "2026-11-15", "Logistics", "upcoming"),
      mkMs("In-house", "2026-11-19", "Buyer", "upcoming"),
    ],
    logs: [
      { date: "2026-07-01", by: "Gaurika", text: "Lab dip R1 submitted. Waiting for Lilly Pulitzer approval.", src: "email" },
      { date: "2026-06-21", by: "Gaurika", text: "Fabric ordered. Amira Pullover 023627.", src: "whatsapp" },
  ...
  },
  {
    ...
    milestones: [
      ...
      mkMs("Lab dip", "2026-04-20", "Factory/Buyer", "done", 0, [
        { label: "R1 Sub", date: "2026-04-20", status: "done" }, { label: "R1 App", date: "2026-04-26", status: "done" },
        { label: "R2 Sub", date: "2026-04-30", status: "done" }, { label: "R2 App", date: "2026-05-05", status: "done" },
      ]),
      mkMs("Fit sample", "2026-04-27", "Factory/Buyer", "done", 0, [
        { label: "R1 Sub", date: "2026-04-27", status: "done" }, { label: "R1 App", date: "2026-05-03", status: "done" },
        { label: "R2 Sub", date: "2026-05-14", status: "done" }, { label: "R2 App", date: "2026-05-20", status: "done" },
      ]),
      ...
      mkMs("GAC", "2026-10-10", "Logistics", "upcoming"),
      mkMs("In-house", "2026-10-14", "Buyer", "upcoming"),
    ],
    logs: [{ date: "2026-06-28", by: "Gaurika", text: "FPT samples sent to SGS lab.", src: "email" }],
    docs: [{ name: "TB_PO_30162201.pdf" }],
  },
];

const TNA_COLS = [
  { id: "fabric-commitment", short: "Fab Commit" }, { id: "fabric-ordered", short: "Fab Order" }, { id: "lab-dip", short: "Lab Dip" }, { id: "fit-sample", short: "Fit" },
  { id: "fob-submission", short: "FOB Sub" }, { id: "fob-approval", short: "FOB App" }, { id: "fpt", short: "FPT" }, { id: "gpt", short: "GPT" },
  { id: "pp-sample", short: "PP" }, { id: "pcd", short: "PCD" }, { id: "ex-factory", short: "X-Fty" }, { id: "gac", short: "GAC" }, { id: "in-house", short: "IH" },
];
...
// Segmented round progress bar — scannable round tracker
function RoundBar({ rounds }) {
  const segColor = (s) => s === "done" ? "bg-emerald-500" : s === "overdue" ? "bg-red-500" : s === "atrisk" ? "bg-amber-500" : "bg-gray-200";
  const txtColor = (s) => s === "done" ? "text-emerald-700" : s === "overdue" ? "text-red-700" : s === "atrisk" ? "text-amber-700" : "text-gray-400";
  return (
    <div className="mt-1.5">
      <div className="flex gap-1">
        {rounds.map((r, i) => (
          <div key={i} className="flex-1 flex flex-col items-center min-w-0">
            <div className={`h-1.5 w-full rounded-full ${segColor(r.status)}`} />
            <div className="mt-1 text-center min-w-0 w-full">
              <div className={`text-[9px] font-semibold truncate ${txtColor(r.status)}`}>{r.label}</div>
              <div className="text-[8px] text-gray-400">{fmt(r.date)}</div>
            </div>
          </div>
        ))}
      </div>
...
function Sidebar({ page, setPage }) {
  const items = [{ id: "dashboard", icon: LayoutDashboard, label: "Dashboard" }, { id: "orders", icon: ListChecks, label: "Orders" }, { id: "buyers", icon: Building2, label: "Buyers" }, { id: "calendar", icon: CalIcon, label: "Calendar" }];
  return (
    <div className="w-[176px] bg-white border-r border-gray-200 flex flex-col py-3 px-2.5 gap-0.5 flex-shrink-0">
      <div className="flex items-center gap-2 px-1.5 mb-4">
        <div className="w-8 h-8 rounded-lg bg-purple-600 flex items-center justify-center flex-shrink-0"><GitBranch className="w-4 h-4 text-white" /></div>
        <span className="text-[15px] font-bold text-gray-900 tracking-tight">TrackTale</span>
      </div>
      {items.map(it => <button key={it.id} onClick={() => setPage(it.id)} className={`flex items-center gap-2.5 px-2.5 h-9 rounded-lg text-[12px] font-medium transition-colors ${page === it.id ? "bg-purple-100 text-purple-700" : "text-gray-500 hover:bg-gray-100"}`}><it.icon className="w-[17px] h-[17px] flex-shrink-0" />{it.label}</button>)}
      <div className="flex-1" />
      <button className="flex items-center gap-2.5 px-2.5 h-9 rounded-lg text-[12px] font-medium text-gray-500 hover:bg-gray-100"><Settings className="w-[17px] h-[17px] flex-shrink-0" />Settings</button>
    </div>
  );
}
...
function Dashboard({ orders, sms, smsProj, onSelect, onNewOrder, onAddTna, onUploadSms, uploadState, onDeletePending, onDeleteOrder }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      <input type="file" accept=".xlsx,.xls,.csv" ref={fileRef} onChange={e => { const f = e.target.files?.[0]; if (f) onUploadSms(f); e.target.value = ""; }} className="hidden" />
      <div className="px-4 py-2.5 border-b border-gray-200 flex items-center justify-between bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 rounded-lg text-[11px] text-gray-700 font-medium hover:bg-gray-200"><Upload className="w-3.5 h-3.5" /> Upload SMS</button>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            {uploadState?.error && <span className="text-[10px] text-red-700 bg-red-50 px-2 py-0.5 rounded font-medium flex items-center gap-1"><AlertTriangle className="w-3 h-3" />{uploadState.error}</span>}
          </div>
          <div className="flex gap-3">
            <div className="flex-1 min-w-0 flex flex-col gap-2">
              ...
              {filteredPending.map(s => (
                <div key={s.id} className={`group bg-white border rounded-lg p-3 border-l-[3px] hover:shadow-sm transition-all ${s.fresh ? "border-emerald-400 border-l-emerald-500 ring-2 ring-emerald-200 shadow-sm" : "border-gray-200 border-l-amber-400"}`}>
                  <div className="flex items-center gap-1.5 mb-1.5"><Chip buyer={s.buyer} /><span className="text-[9px] text-gray-500">{s.season}</span>{s.fresh && <span className="text-[10px] font-bold text-white bg-emerald-500 px-2 py-0.5 rounded-md ml-auto shadow-sm">JUST ADDED</span>}<button onClick={() => onDeletePending(s)} className={`${s.fresh ? "ml-1.5" : "ml-auto"} text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity`} title="Remove style"><Trash2 className="w-3.5 h-3.5" /></button></div>
              ...
              {filteredPending.length === 0 && <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-lg p-8 text-center text-[11px] text-gray-400">All styles added to TNA</div>}
            </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-2">
              ...
              {inTnaStyles.map(s => {
                ...
                return (
                  <div key={s.id} onClick={() => ord && onSelect(ord)} className={`group bg-white border border-gray-200 rounded-lg p-3 border-l-[3px] ${borderC} hover:shadow-sm transition-all ${ord ? "cursor-pointer" : ""}`}>
                    <div className="flex items-center gap-1.5 mb-1.5"><Chip buyer={s.buyer} /><span className="text-[10px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded">In TNA ✓</span><button onClick={e => { e.stopPropagation(); onDeleteOrder(s); }} className="ml-auto text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity" title="Delete order"><Trash2 className="w-3.5 h-3.5" /></button></div>
                    <div className="text-[13px] font-semibold text-gray-900 mb-0.5">{s.code}</div>
                    <div className="text-[11px] text-gray-500 mb-1">{s.desc}</div>
                    <div className="text-[10px] text-gray-400 mb-2">{s.category} · {s.units.toLocaleString()} units</div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(seasonGroups).map(([season, sOrds]) => {
              ...
              const byBuyer = {}; sOrds.forEach(o => { if (!byBuyer[o.buyer]) byBuyer[o.buyer] = []; byBuyer[o.buyer].push(o); });
              return (
                <div key={season} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  {Object.keys(byBuyer).length > 0 && <div className="divide-y divide-gray-50">
                    {Object.entries(byBuyer).map(([b, bOrds]) => {
                      const key = `${season}-${b}`;
                      const isOpen = expandedSeason === key;
                      const bUnits = bOrds.reduce((s, o) => s + o.units, 0);
                      const bOvd = bOrds.filter(o => getHealth(o) === "overdue").length;
                      const bAr = bOrds.filter(o => getHealth(o) === "atrisk").length;
                      return (
                        <div key={b}>
                          <div onClick={() => setExpandedSeason(isOpen ? null : key)} className="px-4 py-2 flex items-center gap-2 cursor-pointer hover:bg-gray-50">
                            {isOpen ? <ChevronDown className="w-3.5 h-3.5 text-gray-400" /> : <ChevronRight className="w-3.5 h-3.5 text-gray-400" />}
                            <Chip buyer={b} />
                            <span className="text-[11px] text-gray-600">{bOrds.length} style{bOrds.length > 1 ? "s" : ""} · {bUnits.toLocaleString()} units</span>
                            <div className="ml-auto flex gap-1">
                              {bOvd > 0 && <span className="text-[9px] font-bold bg-red-100 text-red-700 px-1.5 py-0.5 rounded">{bOvd}d</span>}
                              {bAr > 0 && <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">{bAr}r</span>}
                            </div>
                          </div>
                          {isOpen && <div className="bg-gray-50 px-4 py-1.5">
                            {bOrds.map(o => {
                              const crit = o.milestones.find(m => m.status === "overdue" || m.status === "atrisk");
                              return (
                                <div key={o.id} onClick={() => onSelect(o)} className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-white rounded px-2 -mx-2">
                                  <span className="text-[11px] font-semibold text-gray-900 w-16">{o.po}</span>
                                  <span className="text-[10px] text-gray-500 flex-1">{o.style} · {o.units.toLocaleString()} units</span>
                                  {crit && <span className="text-[9px] text-gray-500">{crit.name}</span>}
                                  <Pill status={getHealth(o) === "overdue" ? "overdue" : getHealth(o) === "atrisk" ? "atrisk" : "done"} planned={crit?.planned} />
                                </div>
                              );
                            })}
                          </div>}
...
function Detail({ order, onBack, onCascade }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      <div className="px-4 py-2.5 border-b border-gray-200 bg-white flex items-center gap-2">
        ...
        <span className="text-[11px] text-gray-400">Orders</span><ChevronRight className="w-3 h-3 text-gray-300" /><span className="text-[11px] text-gray-900 font-semibold">PO {order.po}</span>
      </div>
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => onCascade(order)} className="text-[10px] px-2.5 py-1 rounded-md bg-purple-100 text-purple-700 font-semibold hover:bg-purple-200">Recalculate from commit date</button>
              <button className="text-[10px] px-2.5 py-1 rounded-md bg-gray-100 text-gray-700 font-medium">Edit</button>
            </div>
          </div>
          ...
          {att.map(m => <div key={m.id} className="bg-white border border-gray-200 rounded-lg mb-2 border-l-[3px] border-l-red-500 overflow-hidden">
            ...
            {m.rounds && <div className="px-3 pb-3 ml-5 mr-3"><RoundBar rounds={m.rounds} /></div>}
          </div>)}</>}
...
function TnaTable({ orders, onSelect }) {
  const cellFor = (order, colId) => {
    const m = order.milestones.find(x => x.id === colId);
    if (!m) return <td key={colId} className="px-2 py-2 text-center text-[10px] text-gray-300 border-r border-gray-100">—</td>;
    const bg = m.status === "done" ? "bg-emerald-50" : m.status === "overdue" ? "bg-red-50" : m.status === "atrisk" ? "bg-amber-50" : "bg-white";
    const tx = m.status === "done" ? "text-emerald-700" : m.status === "overdue" ? "text-red-700 font-bold" : m.status === "atrisk" ? "text-amber-700 font-semibold" : "text-gray-500";
    return <td key={colId} className={`px-2 py-2 text-center text-[10px] border-r border-gray-100 ${bg} ${tx}`}>{fmt(m.planned)}</td>;
  };
  return (
    <div className="flex-1 overflow-auto p-4">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100 border-b border-gray-200">
                <th className="sticky left-0 z-10 bg-gray-100 px-3 py-2 text-left text-[10px] font-bold text-gray-600 border-r border-gray-200 min-w-[130px]">Order</th>
                {TNA_COLS.map(c => <th key={c.id} className="px-2 py-2 text-center text-[10px] font-semibold text-gray-500 border-r border-gray-100 whitespace-nowrap min-w-[64px]">{c.short}</th>)}
              </tr>
            </thead>
            <tbody>
              {orders.map(o => {
                const h = getHealth(o);
                const edge = h === "overdue" ? "border-l-red-500" : h === "atrisk" ? "border-l-amber-500" : "border-l-emerald-500";
                return (
                  <tr key={o.id} onClick={() => onSelect(o)} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                    <td className={`sticky left-0 z-10 bg-white px-3 py-2 border-r border-gray-200 border-l-[3px] ${edge}`}>
                      <div className="text-[11px] font-semibold text-gray-900">PO {o.po}</div>
                      <div className="flex items-center gap-1 mt-0.5"><Chip buyer={o.buyer} /></div>
                    </td>
                    {TNA_COLS.map(c => cellFor(o, c.id))}
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="flex items-center gap-4 mt-3 text-[10px] text-gray-500">
        <span className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-emerald-50 border border-emerald-200" /> Done</span>
        <span className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-red-50 border border-red-200" /> Overdue</span>
        <span className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-amber-50 border border-amber-200" /> At risk</span>
        <span className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-white border border-gray-200" /> Upcoming</span>
        <span className="ml-auto text-gray-400">PO column stays fixed while you scroll milestones →</span>
      </div>
    </div>
  );
}

function OrdersPage({ orders, onSelect, onNewOrder }) {
  ...
  const views = [{ id: "board", icon: LayoutGrid, label: "Board" }, { id: "table", icon: Rows3, label: "Table" }, { id: "calendar", icon: CalIcon, label: "Calendar" }];
    ...
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="px-4 py-2.5 border-b border-gray-200 bg-white flex items-center gap-2">
        <div className="flex bg-gray-100 rounded-lg p-0.5">
          {views.map(v => <button key={v.id} onClick={() => setView(v.id)} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${view === v.id ? "bg-white text-purple-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}><v.icon className="w-3.5 h-3.5" /> {v.label}</button>)}
        </div>
      ...
      {view === "board" && <div className="flex-1 overflow-auto p-4">
      ...
      {view === "table" && <TnaTable orders={filtered} onSelect={onSelect} />}
...
function AddOrder({ onSave, onCancel, prefill }) {
  ...
  const cascade = f.fc ? cascadeDates(f.fc, f.buyer, f.ih) : {};
  const xf = cascade["ex-factory"];
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6 max-w-lg">
          ...
          {step === 4 && <div>
            <div className="text-lg font-semibold text-gray-900 mb-1">Review TNA</div><div className="text-[12px] text-gray-500 mb-4">All dates auto-calculated from fabric commitment</div>
            ...
            {f.fc && <div className="bg-white border border-gray-200 rounded-lg overflow-hidden mb-3">
              <div className="px-3 py-1.5 bg-gray-100 text-[10px] font-semibold text-gray-500 border-b border-gray-200">Calculated milestones</div>
              {TNA_COLS.map(c => <div key={c.id} className="flex items-center justify-between px-3 py-1.5 border-b border-gray-50 text-[11px]"><span className="text-gray-700">{c.short}</span><span className="text-gray-900 font-medium">{cascade[c.id] ? fmt(cascade[c.id]) : "—"}</span></div>)}
            </div>}
            <div className="p-3 bg-amber-100 rounded-lg text-[11px] text-amber-800 font-medium flex items-start gap-1.5"><AlertCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" /><span>Dates calculated using {f.buyer} rules ({f.buyer === "Talbots" ? 59 : 49}d X-Fty to IH) and the mentor's TNA formulas. Adjustable after saving.</span></div>
            <div className="flex justify-between mt-6"><button onClick={() => setStep(3)} className="text-[12px] text-gray-500 font-medium">Back</button><button onClick={() => onSave({ ...f, cascade, units: parseInt(f.units) || 0 })} className="px-5 py-2 bg-purple-600 text-white text-[12px] font-semibold rounded-lg flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> Save order</button></div>
          </div>}
        </div>
      </div>
    </div>
...
export default function TrackTaleGM() {
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // { type: 'pending'|'order', style, order }

  const handleAddTna = (s) => { setPrefill(s); setNewOrder(true); };

  // Delete a pending SMS style outright (e.g. a junk row)
  const deletePendingStyle = (style) => {
    setSms(prev => prev.filter(s => s.id !== style.id));
    setConfirmDelete(null);
  };

  // Delete an in-TNA order. mode: 'pending' sends style back to Pending, 'remove' deletes both.
  const deleteOrder = (style, mode) => {
    const ord = orders.find(o => o.po === style.code || o.style === style.code);
    if (ord) setOrders(prev => prev.filter(o => o.id !== ord.id));
    if (mode === "pending") {
      setSms(prev => prev.map(s => s.id === style.id ? { ...s, inTna: false, fresh: false } : s));
    } else {
      setSms(prev => prev.filter(s => s.id !== style.id));
    }
    if (selOrder && ord && selOrder.id === ord.id) setSelOrder(null);
    setConfirmDelete(null);
  };

  const handleUploadSms = async (file) => {
    ...
    try {
      ...
      if (!rows.length) { setUploadState({ error: "Sheet is empty" }); return; }
      const keys = Object.keys(rows[0]);
      const findKey = (cands) => keys.find(k => cands.some(c => k.toLowerCase().replace(/[^a-z0-9]/g, "").includes(c)));
      const codeKey = findKey(["stylecode", "style", "code", "styleno", "stylenumber"]);
      const descKey = findKey(["description", "desc", "stylename", "name"]);
      const catKey = findKey(["category", "cat", "type", "class"]);
      const unitKey = findKey(["units", "unit", "qty", "quantity", "orderqty", "projectedunits"]);
      if (!codeKey) { setUploadState({ error: "No style code column found" }); setTimeout(() => setUploadState(null), 5000); return; }
      const parsed = rows.map((r, i) => {
        const rawUnits = unitKey ? String(r[unitKey]).replace(/[^0-9.]/g, "") : "";
        return { id: `up-${Date.now()}-${i}`, code: String(r[codeKey]).trim(), desc: descKey ? String(r[descKey]).trim() : "—", category: catKey ? String(r[catKey]).trim() : "—", units: rawUnits ? parseInt(rawUnits) : 0, buyer: "Lilly Pulitzer", season: "SP27", inTna: false, fresh: true };
      }).filter(s => {
        if (!s.code || s.code === "") return false;
        const junk = /^(subtotal|total|grand total|sum|header|style code)$/i.test(s.code) || /^(subtotal|total|grand total)$/i.test(s.desc);
        return !junk;
      });
      if (!parsed.length) { setUploadState({ error: "No valid style rows found in that sheet" }); setTimeout(() => setUploadState(null), 5000); return; }
      setSms(prev => [...parsed, ...prev.map(s => ({ ...s, fresh: false }))]);
      setUploadState({ success: `${file.name} — ${parsed.length} styles added to Pending TNA` });
      setTimeout(() => setUploadState(null), 4500);
    } catch (err) {
      setUploadState({ error: "Could not read file. Use .xlsx, .xls, or .csv" });
      setTimeout(() => setUploadState(null), 5000);
    }
  };

  const handleCascade = (order) => {
    const c = cascadeDates(order.fabricCommit, order.buyer, order.ih);
    setOrders(prev => prev.map(o => {
      if (o.id !== order.id) return o;
      const updated = o.milestones.map(m => c[m.id] ? { ...m, planned: c[m.id] } : m);
      const newSel = { ...o, milestones: updated, xfty: c["ex-factory"] || o.xfty };
      if (selOrder && selOrder.id === o.id) setSelOrder(newSel);
      return newSel;
    }));
  };

  const handleSave = (f) => {
    const c = f.cascade || cascadeDates(f.fc, f.buyer, f.ih);
    const o = { id: Date.now().toString(), po: f.po, buyer: f.buyer, style: f.style || f.po, season: f.season, cycle: parseInt(f.cycle) || 130, fabricCommit: f.fc, xfty: c["ex-factory"], ih: f.ih, units: f.units,
      milestones: [
        mkMs("Fabric commitment", c["fabric-commitment"], "Merchandiser", "upcoming", 12),
        mkMs("Fabric ordered", c["fabric-ordered"], "Merchandiser", "upcoming", 11),
        mkMs("Lab dip", c["lab-dip"], "Factory/Buyer", "upcoming", 9),
        mkMs("Fit sample", c["fit-sample"], "Factory/Buyer", "upcoming", 7),
        mkMs("FOB submission", c["fob-submission"], "Merchandiser", "upcoming", 5),
        mkMs("FOB approval", c["fob-approval"], "Buyer", "upcoming", 4),
        mkMs("FPT", c["fpt"], "Factory/Lab", "upcoming", 3),
        mkMs("GPT", c["gpt"], "Factory/Lab", "upcoming", 2),
        mkMs("PP sample", c["pp-sample"], "Factory/Buyer", "upcoming", 1),
        mkMs("PCD", c["pcd"], "Factory", "upcoming"),
        mkMs("Ex-factory", c["ex-factory"], "Factory", "upcoming"),
        mkMs("GAC", c["gac"], "Logistics", "upcoming"),
        mkMs("In-house", c["in-house"], "Buyer", "upcoming"),
      ],
      logs: [{ date: new Date().toISOString().split("T")[0], by: "Gaurika", text: `Order created. ${f.buyer} ${f.po}, ${f.season}.`, src: "email" }], docs: [] };
    setOrders(p => [...p, o]);
  ...
  return (
    <div className="flex h-[640px] bg-[#F0F0F5] rounded-xl overflow-hidden border border-gray-300 shadow-sm relative" style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      {uploadState?.success && <div className="absolute top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white text-[12px] font-semibold rounded-lg shadow-lg"><CheckCircle2 className="w-4 h-4" />{uploadState.success}</div>}
      {uploadState?.loading && <div className="absolute top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2.5 bg-gray-800 text-white text-[12px] font-semibold rounded-lg shadow-lg">Reading sheet…</div>}

      {confirmDelete && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/30" onClick={() => setConfirmDelete(null)}>
          <div className="bg-white rounded-xl shadow-xl w-[340px] p-5" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-3 mb-3">
              <div className="w-9 h-9 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0"><Trash2 className="w-4 h-4 text-red-600" /></div>
              <div>
                <div className="text-[14px] font-bold text-gray-900">
                  {confirmDelete.type === "pending" ? "Remove this style?" : "Delete this order?"}
                </div>
                <div className="text-[11px] text-gray-500 mt-0.5">
                  {confirmDelete.style.code} · {confirmDelete.style.desc}
                </div>
              </div>
            </div>
            ...
            {confirmDelete.type === "pending" ? (
              <>
                <p className="text-[12px] text-gray-600 mb-4">This removes the style from Pending TNA. You can always upload the sheet again.</p>
                <div className="flex gap-2">
                  <button onClick={() => setConfirmDelete(null)} className="flex-1 text-[12px] font-semibold px-3 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">Cancel</button>
                  <button onClick={() => deletePendingStyle(confirmDelete.style)} className="flex-1 text-[12px] font-semibold px-3 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">Remove</button>
                </div>
              </>
            ) : (
              <>
                <p className="text-[12px] text-gray-600 mb-4">What should happen to this style once its order is deleted?</p>
                <div className="space-y-2">
                  <button onClick={() => deleteOrder(confirmDelete.style, "pending")} className="w-full text-left px-3 py-2.5 rounded-lg border border-gray-200 hover:border-purple-300 hover:bg-purple-50 transition-colors">
                    <div className="text-[12px] font-semibold text-gray-900">Move back to Pending TNA</div>
                    <div className="text-[10px] text-gray-500">Keep the style, just undo the tracking</div>
                  </button>
                  <button onClick={() => deleteOrder(confirmDelete.style, "remove")} className="w-full text-left px-3 py-2.5 rounded-lg border border-gray-200 hover:border-red-300 hover:bg-red-50 transition-colors">
                    <div className="text-[12px] font-semibold text-red-700">Remove completely</div>
                    <div className="text-[10px] text-gray-500">Delete the order and the style</div>
                  </button>
                </div>
                <button onClick={() => setConfirmDelete(null)} className="w-full mt-3 text-[12px] font-semibold px-3 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">Cancel</button>
              </>
            )}
          </div>
        </div>
      ...
      {newOrder ? <AddOrder onSave={handleSave} onCancel={() => { setNewOrder(false); setPrefill(null); }} prefill={prefill} /> :
       selOrder ? <Detail order={selOrder} onBack={() => setSelOrder(null)} onCascade={handleCascade} /> :
       page === "dashboard" ? <Dashboard orders={orders} sms={sms} smsProj={SMS_PROJ} onSelect={setSelOrder} onNewOrder={() => setNewOrder(true)} onAddTna={handleAddTna} onUploadSms={handleUploadSms} uploadState={uploadState} onDeletePending={s => setConfirmDelete({ type: "pending", style: s })} onDeleteOrder={s => setConfirmDelete({ type: "order", style: s })} /> :
       page === "orders" ? <OrdersPage orders={orders} onSelect={setSelOrder} onNewOrder={() => setNewOrder(true)} /> :
