  ...
  const handleUploadSms = async (file) => {
    ...
    try {
      ...
      const parsed = rows.map((r, i) => {
        ...
        return { id: `up-${Date.now()}-${i}`, code: String(r[codeKey]).trim(), desc: descKey ? String(r[descKey]).trim() : "—", category: catKey ? String(r[catKey]).trim() : "—", units: rawUnits ? parseInt(rawUnits) : 0, buyer: "Lilly Pulitzer", season: "SP27", inTna: false };
      }).filter(s => s.code && s.code !== "");
      if (!parsed.length) { setUploadState({ error: "No valid style rows found" }); setTimeout(() => setUploadState(null), 5000); return; }
      setSms(prev => [...prev, ...parsed]);
      setUploadState({ success: `${file.name} — ${parsed.length} styles extracted` });
