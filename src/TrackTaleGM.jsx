...
import { LayoutDashboard, ListChecks, Building2, Calendar as CalIcon, Settings, Search, Bell, Plus, ChevronRight, ChevronDown, AlertCircle, Clock, CheckCircle2, ArrowLeft, MessageSquare, FileText, Upload, Check, User, GitBranch, Table2, ChevronLeft, LayoutGrid, Rows3, AlertTriangle } from "lucide-react";
...
function Dashboard({ orders, sms, smsProj, onSelect, onNewOrder, onAddTna, onUploadSms, uploadState }) {
          ...
          <div className="flex gap-3">
            <div className="flex-1 min-w-0 flex flex-col gap-2">
              ...
              {filteredPending.map(s => (
                <div key={s.id} className={`bg-white border rounded-lg p-3 border-l-[3px] hover:shadow-sm transition-all ${s.fresh ? "border-emerald-400 border-l-emerald-500 ring-2 ring-emerald-200 shadow-sm" : "border-gray-200 border-l-amber-400"}`}>
                  <div className="flex items-center gap-1.5 mb-1.5"><Chip buyer={s.buyer} /><span className="text-[9px] text-gray-500">{s.season}</span>{s.fresh && <span className="text-[10px] font-bold text-white bg-emerald-500 px-2 py-0.5 rounded-md ml-auto shadow-sm">JUST ADDED</span>}</div>
  ...
  return (
    <div className="flex h-[640px] bg-[#F0F0F5] rounded-xl overflow-hidden border border-gray-300 shadow-sm relative" style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      {newOrder ? <AddOrder onSave={handleSave} onCancel={() => { setNewOrder(false); setPrefill(null); }} prefill={prefill} /> :
       ...
       page === "dashboard" ? <Dashboard orders={orders} sms={sms} smsProj={SMS_PROJ} onSelect={setSelOrder} onNewOrder={() => setNewOrder(true)} onAddTna={handleAddTna} onUploadSms={handleUploadSms} uploadState={uploadState} /> :
