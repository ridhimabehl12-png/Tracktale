...
function Dashboard({ orders, sms, smsProj, onSelect, onNewOrder, onAddTna }) {
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="px-4 py-2.5 border-b border-gray-200 flex items-center justify-between bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 rounded-lg text-[11px] text-gray-700 font-medium hover:bg-gray-200"><Upload className="w-3.5 h-3.5" /> Upload SMS</button>
...
export default function TrackTaleGM() {
  ...
  return (
    <div className="flex h-[640px] bg-[#F0F0F5] rounded-xl overflow-hidden border border-gray-300 shadow-sm" style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      {newOrder ? <AddOrder onSave={handleSave} onCancel={() => { setNewOrder(false); setPrefill(null); }} prefill={prefill} /> :
       ...
       page === "dashboard" ? <Dashboard orders={orders} sms={sms} smsProj={SMS_PROJ} onSelect={setSelOrder} onNewOrder={() => setNewOrder(true)} onAddTna={handleAddTna} /> :
